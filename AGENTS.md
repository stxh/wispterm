...
| `global_state_guard` | top-level `g_*` / `threadlocal` in the watched integration/session files | AppWindow **66**, input **48**, overlays **32**, assistant/conversation/session **15** | put new state in a state struct (`appwindow/state.zig`, …) |
| `import_hub_guard` | `pub const X = @import(...)` re-exports in `AppWindow.zig` | **17** | import the real module directly, not via `AppWindow.X` |
| `side_effect_guard` | direct `g_force_rebuild` / `g_cells_valid` writes in the watched integration/session files | AppWindow **44**, input **81**, overlays **0**, assistant/conversation/session **0** | return a `UiEffect`, land it through `AppWindow.applyUiEffect` |
...
These features are WispTerm identity features. They must survive every merge from `main`. If a merge overwrites or removes them, restore them before committing the merge. Full implementation details are in [docs/persistent-ui-behaviors.md](docs/persistent-ui-behaviors.md).

1. **Icon-only sidebar with hover tooltip.** The left sidebar is 48px wide and shows one centered emoji icon per tab (五行 series — 金🪙 / 木🌲 / 水🌊 / 火🔥 / 土🌍 — for terminal tabs; kind-specific emoji for AI/SSH/skill/port-forward tabs). No tab titles or numbers in the sidebar row. On mouse hover, a floating tooltip popup shows the tab title, current working directory, and git branch. The tooltip has an opaque background and auto-dismisses after 3 seconds. The tooltip is rendered in the overlay pass (`renderSidebarTooltipOverlay`) so it is not overwritten by terminal cells. **`sidebarNeedsAnimation()` must be called from `overlays.anyOverlayActive()`** so the event-driven render loop keeps ticking during the dwell period — without this the tooltip never appears. When the sidebar is hidden, an 8px indicator bar on the left edge shows tab segments (active tab in accent color).

2. **Right-click sidebar tab → always show close confirmation dialog.** Right-clicking a tab icon in the sidebar **always** opens a close-confirmation dialog (`overlays.closeConfirmOpen`) — it never directly closes the tab. The dialog variant is `.running_program` when the tab has a running program, `.window_generic` otherwise. Left-click switches tabs. Middle-click uses `requestCloseTabGesture` which only confirms when a program is running.
...
4. **Terminal process exit closes the tab/window.** When the terminal's child process exits (PTY EOF / child exit), the tab is automatically closed. If it was the last tab, the window closes. This is handled by `sweepExitedSurfaces()` which runs unconditionally at the top of the main loop (before `pollEvents` and before the render gate). The PTY read thread calls `surface.markExited()` which sets `io_state = .exited` and calls `window_backend.postWakeup()` to wake the main loop.

5. **File-explorer toggle on the titlebar.** A folder icon (📂 or MDL2 `0xE8B7`) sits on the window titlebar, immediately to the right of the sidebar toggle icon (`TITLEBAR_FOLDER_W = 46`). Clicking it toggles the global file explorer panel — the same action as `Ctrl+Shift+Alt+E`. The file explorer shows the file tree for the currently active terminal's working directory. When the explorer is open, the icon uses the accent color. **Do NOT put this toggle in the sidebar.**

When merging from `main`, check that all five features are intact: grep for `renderSidebarTooltipOverlay`, `sidebarNeedsAnimation`, `sessionLauncherOpen`, `sweepExitedSurfaces`, and `TITLEBAR_FOLDER_W`. If any are missing, restore them from the `dev` branch history before committing the merge.
...
The project pins `minimum_zig_version = "0.15.2"` (see `build.zig.zon`); do **not** build with a newer Zig (0.16/0.17) — the std build-API changed and every `build.zig` fails to compile. The helper `build-env.ps1` sets the env vars below and can run the build. Two Windows-specific gotchas: (1) **Zig 0.15.2 panics** (`assert` in `Build.Step.Run.convertPathArg`) when a run step's `cwd` and an argv path land on different drive letters, because `std.fs.path.relative` returns an absolute path across drives. Keep the zig global cache on the same drive as the source via `ZIG_GLOBAL_CACHE_DIR` (e.g. `E:\zig-global-cache`; the seeded GitHub deps already live there). (2) GitHub dependencies (zigimg, uucode, imgui, …) must be fetched through the local proxy `http://localhost:11080` (Zig's own HTTP-proxy CONNECT is broken on 0.15.2, so fetch with `curl` and seed the cache, or set `HTTP_PROXY`/`HTTPS_PROXY`). Once the global cache is warm the build runs offline.
